...
ADA usa **dos cerebros** para responder en el chat. Si el chat no responde o da timeout, suele ser porque uno o ambos fallan.
...
| **Error 4xx/5xx** | Revisa logs: `docker logs ada_agent_core 2>&1`. Se imprime `GEMINI FAIL` o `GEMINI ERROR` con código y mensaje. |
...
4. **¿Qué dice agent-core en los logs?**  
   `docker logs ada_agent_core 2>&1 | tail -50` — busca `ADV_BRAIN FAIL`, `OLLAMA CHAT FAIL`, `OLLAMA ERROR`.
...
## 5. Variables de entorno relevantes (agent-core)
...
| `OLLAMA_CHAT_TIMEOUT` | Timeout en segundos para /api/chat (default 120). |
